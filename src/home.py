...
st.title("Environmental Emissions Estimator for Distributed Data Jobs using Runtime Information")
...
upload_runs = st.sidebar.file_uploader(
    "Upload your own Trace Runs with utilisation information", help="Must be in same format as scout data")
upload_power_curves = st.sidebar.file_uploader(
...
if upload_runs is not None:
    try:
        ...
        with st.spinner("Training model"):
            util_reg, time_reg, predicted_cpu_time, encoded_features = utils.train_gbr(
                inputdf)
        success_msg()
...
def get_features(df):
    ...
    for ind, val in enumerate(features):
        if df[val].dtype == 'int64':
            feature_input[ind] = st.number_input(
                label=format_string(features[ind]), min_value=4, step=1)
        elif df[val].dtype == 'float64':
            feature_input[ind] = st.number_input(
                label=format_string(features[ind]), min_value=0)
        elif df[val].dtype == 'object':
            possible_vals = df[val].unique()
            if len(possible_vals) < 25:
                feature_input[ind] = st.radio(label=format_string(
                    features[ind]), options=possible_vals, horizontal=True)
            else:
    ...
    CARBON_INTENSITY = grid_carbon[grid_carbon['Region']
                                   == location]["CO2"].values[0]
    st.markdown("""---""")

    for ind, val in enumerate(user_features):
        if df[val].dtype == 'int64':
            feature_input.append(st.number_input(
                label=format_string(user_features[ind]), min_value=2, step=1))
        elif df[val].dtype == 'float64':
            feature_input.append(st.number_input(
                label=format_string(user_features[ind]), min_value=0))
        elif df[val].dtype == 'object':
            possible_vals = df[val].unique()
            if len(possible_vals) < 25:
                feature_input.append(st.radio(label=format_string(
                    user_features[ind]), options=possible_vals, horizontal=True))
            else:
                feature_input.append(st.selectbox(label=format_string(
                    user_features[ind]), options=possible_vals))
    feature_inputdf = pd.DataFrame(
        columns=features+user_features, data=[feature_input])
...
with st.expander("Override preset data"):
    ...
    PUE = st.number_input("Power Usage Effectiveness of Data Center",
                          min_value=1.0, value=1.135, help="Minumum value of 1")
    intensity_override = st.number_input(
        "Grid Carbon Intensity (g/CO2/kWh)", value=0.0, min_value=0.0, help="Override applies if value not set to 0")
...
def get_power(power_df, e_util, e_time, instance_type, nodes):
    ...
    intercept = power_df.loc[power_df['instance_type']
                             == instance, 'intercept'].values[0]
    gradient = power_df.loc[power_df['instance_type']
                            == instance, 'slope'].values[0]
    hourpower_single = (gradient * e_util) + intercept
    hourpower = hourpower_single*nodes
    max_power = power_df.loc[power_df['instance_type']
                             == instance, 'max_power'].values[0]
    powerused = (hourpower/3600)*e_time
    pue_power = powerused*PUE
    return (pue_power, max_power)


def get_carbon(powerused):
    carbon = (powerused*CARBON_INTENSITY)/1000
    return carbon
...
def predict_with_trace(feature_inputdf, predicted_cpu_time):
    ...
    match = predicted_cpu_time.query(
        "job_input == @job_input and engine_type == @engine_type and data_size == @data_size and node_count == @node_count and instance_type == @instance_type")
    if match.empty:
        return None
    return match
...
def show_match_prediction(match_util, match_time, match_power_result, match_carbon_result, e_util, e_time, power_result, carbon_result):
    ...
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Resource Utilisation", f"{np.round(match_util)}%",
                delta=f"{round(((match_util-e_util[0])/e_util[0])*100,2)}%")
    col2.metric("Runtime Duration", utils.time_format(seconds=match_time),
                delta=f'{round(((match_time-e_time[0])/e_time[0])*100,2)}%')
    col3.metric("Power Usage", f"{round(match_power_result,2)}w",
                delta=f"{round(((match_power_result-power_result[0])/power_result[0])*100,2)}%")
    col4.metric("Environmental Footprint", f"{round(match_carbon_result,2)} gCO2eq",
                delta=f"{round(((match_carbon_result-carbon_result)/carbon_result)*100,2)}%")
    st.caption(
        "Deltas show the percentage change of the metric compared to the initial estimate ")


if run_prediction:
    nodes = feature_inputdf['node_count'].values[0]
    chosen_features = feature_inputdf.copy()
    e_util, e_time, instance_type = utils.predict_gbr(
        util_reg, time_reg, feature_inputdf, encoded_features.copy())
    power_result, max_power = get_power(
        power_df, e_util, e_time, instance_type, nodes)
    ...
    if match is not None:
        match_util, match_time = np.array(
            match['true_cpu'].values[0]), np.array(match['true_time'].values[0])
        match_power_result, _ = get_power(
            power_df, match_util, match_time, instance_type, nodes)
        match_carbon_result = get_carbon(match_power_result)
        show_match_prediction(match_util, match_time, match_power_result,
                              match_carbon_result, e_util, e_time, power_result, carbon_result)
...
if recommendation:
    ...
    constrained_df = predicted_cpu_time[(predicted_cpu_time['job_input'] == selected_job) & (
        predicted_cpu_time['engine_type'] == selected_engine) & (predicted_cpu_time['data_size'] == selected_size)]

    all_df = constrained_df.merge(
        power_df[['instance_type', 'max_power', 'slope', 'intercept']])

    all_df["estimated_gCO2eq"] = ((((all_df['slope']*all_df['true_cpu']+all_df['intercept'])/3600)
                                  * all_df['true_time']*all_df['node_count'])*PUE*CARBON_INTENSITY)/1000
    ...
    index = pd.MultiIndex.from_product([job_input, engine_type, data_size, node_count, cluster_type], names=[
                                       "job_input", "engine_type", "data_size", "node_count", "instance_type"])
    ...
    for col in configs_df.columns:
        if col == 'node_count':
            ...
            configs_df[col] = encoded_features[col].inverse_transform(
                configs_df[col])
    configs_power = configs_df.merge(
        power_df[['instance_type', 'max_power', 'slope', 'intercept']])
    configs_power["e_util"] = e_util_configs
    configs_power["e_time"] = e_time_configs
    configs_power["estimated_power"] = (((configs_power['slope']*configs_power['e_util'] +
                                        configs_power['intercept'])/3600)*configs_power['e_time']*configs_power['node_count'])
    configs_power["estimated_gCO2eq"] = (
        configs_power["estimated_power"]*PUE*CARBON_INTENSITY)/1000
